...
*当前仓库以 Windows + PowerShell 场景为主，使用 `.env` 和 `run.ps1` 启动本地代理，再将本地 URL 和 key 配置给 Cherry Studio、Cursor、Cline、OpenAI SDK 等兼容工具。*
...
可以通过 `.env` 调整连接池、超时、重试和主动 token 刷新行为：

```env
TOKEN_AUTO_REFRESH_ENABLED=true
TOKEN_AUTO_REFRESH_CHECK_INTERVAL=300
TOKEN_AUTO_REFRESH_WINDOW=300
...
- 主动刷新只会在 token 已存在且即将过期时触发，不会主动向上游做健康探测。
- `/health` 会返回 token 到期状态、最近刷新失败次数、错误分类统计，以及当前共享 HTTP client 配置，便于排查本地代理不稳定问题。
