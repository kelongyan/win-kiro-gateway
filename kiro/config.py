...
def load_runtime_env(env_file: str = ".env", override: bool = False) -> None:
    """
    显式加载运行时环境变量。

    默认情况下只在应用启动阶段调用，而不是在导入模块时自动执行，
    这样可以避免测试或其他导入方被本地 `.env` 污染。

    Args:
        env_file: 要加载的环境文件路径
        override: 是否允许 `.env` 覆盖现有进程环境变量
    """
...
def _parse_csv_env_list(raw_value: str) -> List[str]:
    """
    将逗号分隔的环境变量解析为字符串列表。
...
# Server host (default: 127.0.0.1 - local connections only)
# Use "0.0.0.0" only when you explicitly need LAN access
...
# Intentionally defaults to empty: users must configure it explicitly.
PROXY_API_KEY: str = os.getenv("PROXY_API_KEY", "").strip()

# Upstream provider selection
UPSTREAM_PROVIDER: str = os.getenv("UPSTREAM_PROVIDER", "kiro").strip().lower()

# Browser CORS allowlist for local usage.
# Desktop tools usually don't need CORS, but browser clients do.
...
CORS_ALLOWED_ORIGINS: List[str] = (
    _parse_csv_env_list(_raw_cors_allowed_origins)
...
GLM_API_BASE_URL: str = os.getenv("GLM_API_BASE_URL", "https://open.bigmodel.cn/api/paas/v4")
...
HIDDEN_FROM_LIST: List[str] = ["auto"] if UPSTREAM_PROVIDER == "kiro" else ["auto", "claude-3.7-sonnet"]
...
# AGGRESSIVE: Completely disable connection reuse to prevent SSL errors (DECRYPTION_FAILED_OR_BAD_RECORD_MAC).
# AWS closes connections unpredictably, causing SSL state mismatches when reusing connections.
# Setting keepalive to 0 forces fresh connections for each request, eliminating reuse errors.
# Performance impact is minimal for non-streaming requests (adds ~100-300ms TCP/SSL handshake).
# Streaming requests already use Connection: close header (issue #38).
HTTP_MAX_CONNECTIONS: int = _parse_positive_int_env("HTTP_MAX_CONNECTIONS", 100)
HTTP_MAX_KEEPALIVE_CONNECTIONS: int = _parse_positive_int_env("HTTP_MAX_KEEPALIVE_CONNECTIONS", 0)
HTTP_KEEPALIVE_EXPIRY: float = _parse_positive_float_env("HTTP_KEEPALIVE_EXPIRY", 0.0)
