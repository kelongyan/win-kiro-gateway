...
def build_kiro_payload(
    ...
    Args:
        ...
        resolved_model_id: 路由层已解析好的最终模型 ID；为空时保持原有解析逻辑
    ...
    # 优先使用路由层统一解析后的模型 ID，避免展示层与执行层分叉。
    # 为保持兼容，未传入时仍回退到原有的本地解析逻辑。
    model_id = resolved_model_id or get_model_id_for_kiro(request_data.model, HIDDEN_MODELS)
