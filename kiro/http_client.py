...
class KiroHttpClient:
    ...
    async def _close_retry_response(self, response: httpx.Response) -> None:
        """
        关闭不会再返回给调用方的响应对象。

        流式请求在 `stream=True` 时会先拿到响应头再逐步读取 body。
        如果某次重试前不关闭这类失败响应，会留下未释放连接。

        Args:
            response: 本轮不会继续使用的响应对象
        """
        try:
            await response.aclose()
        except Exception as e:
            logger.debug(f"Failed to close retry response: {e}")
    ...
    async def request_with_retry(
        ...
        Returns:
            httpx.Response。成功时返回 200 响应；若 HTTP 状态码在重试后仍失败，
            则返回最后一次上游响应，由路由层按真实状态码继续处理。
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 403/429/5xx 属于“有响应的失败”，最后一次尝试时要把真实响应交还上层，
                # 不能伪装成网络超时，否则会把限流、鉴权失败、服务端错误误报成 504。

                # 403 - token expired, refresh and retry
                if response.status_code == 403:
                    if attempt < max_retries - 1:
                        logger.warning(f"Received 403, refreshing token (attempt {attempt + 1}/{max_retries})")
                        await self._close_retry_response(response)
                        await self.auth_manager.force_refresh()
                        continue
                    logger.warning(f"Received 403 after {max_retries} attempts, returning last response")
                    return response
                ...
                if response.status_code == 429:
                    if attempt < max_retries - 1:
                        ...
                        continue
                    logger.warning(f"Received 429 after {max_retries} attempts, returning last response")
                    return response
                ...
                if 500 <= response.status_code < 600:
                    if attempt < max_retries - 1:
                        ...
                        continue
                    logger.warning(
                        f"Received {response.status_code} after {max_retries} attempts, returning last response"
                    )
                    return response
