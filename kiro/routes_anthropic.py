...
async def messages(
    ...
    logger.debug(
        f"Resolved Anthropic model '{request_data.model}' -> '{model_resolution.internal_id}' "
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            logger.warning(
                f"HTTP {error_result.status_code} - POST /v1/messages - {error_result.user_message[:100]}"
            )
