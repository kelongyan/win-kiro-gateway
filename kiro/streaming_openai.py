...
from kiro.streaming_shared import (
    build_deduplicated_tool_calls,
    detect_content_truncation,
    log_content_truncation,
    save_openai_truncation_state,
)
...
async def stream_kiro_to_openai_internal(
    ...
    try:
        ...
        # 聚合所有 tool call，并统一判断正文是否被截断。
        all_tool_calls = build_deduplicated_tool_calls(
            full_content,
            tool_calls_from_stream,
            parse_tool_calls=parse_bracket_tool_calls,
            dedupe_tool_calls=deduplicate_tool_calls,
        )
        content_was_truncated = detect_content_truncation(
            stream_completed_normally=stream_completed_normally,
            full_content=full_content,
            has_tool_output=bool(all_tool_calls)
        )
        log_content_truncation(content_was_truncated, full_content)
        ...
        save_openai_truncation_state(all_tool_calls, content_was_truncated, full_content)
