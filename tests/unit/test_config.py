...
class TestServerHostConfig:
    ...
    def test_default_server_host_is_127_0_0_1(self):
        """
        What it does: Verifies that SERVER_HOST defaults to 127.0.0.1.
        Purpose: Ensure that localhost-only binding is used when no environment variable is set.
        """
        ...
        with patch.dict(os.environ, {}, clear=False):
            ...
            assert config_module.DEFAULT_SERVER_HOST == "127.0.0.1"
...
class TestFallbackModelsConfig:
    ...
    def test_fallback_models_contain_claude_models(self):
        ...
        print("Verification: Contains provider-appropriate fallback models...")
        has_supported_model = any(("claude" in mid.lower()) or (mid.lower() == "glm-5") for mid in model_ids)
        assert has_supported_model, "No supported fallback models in fallback list"
...
class TestFallbackModelsIntegration:
    ...
    async def test_fallback_models_work_with_model_resolver(self):
        ...
        print("\nAction: Testing normalization with provider-specific fallback models...")
        if FALLBACK_MODELS == [{"modelId": "glm-5"}]:
            test_cases = [
                ("glm-5", "glm-5"),
            ]
        else:
...
class TestUpstreamProviderConfig:
    """Tests for upstream provider and GLM configuration."""

    def test_upstream_provider_defaults_to_kiro(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("UPSTREAM_PROVIDER", None)
            import importlib
            import kiro.config as config_module
            importlib.reload(config_module)
            assert config_module.UPSTREAM_PROVIDER == "kiro"

    def test_upstream_provider_reads_glm(self):
        with patch.dict(os.environ, {"UPSTREAM_PROVIDER": "GLM"}):
            import importlib
            import kiro.config as config_module
            importlib.reload(config_module)
            assert config_module.UPSTREAM_PROVIDER == "glm"
            assert config_module.FALLBACK_MODELS == [{"modelId": "glm-5"}]

    def test_glm_api_base_url_default(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("GLM_API_BASE_URL", None)
            import importlib
            import kiro.config as config_module
            importlib.reload(config_module)
            assert config_module.GLM_API_BASE_URL == "https://open.bigmodel.cn/api/paas/v4"


class TestProxyApiKeyConfig:
    """Tests for PROXY_API_KEY configuration."""

    def test_proxy_api_key_defaults_to_empty(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("PROXY_API_KEY", None)
            import importlib
            import kiro.config as config_module
            importlib.reload(config_module)
            assert config_module.PROXY_API_KEY == ""

    def test_proxy_api_key_reads_from_environment(self):
        with patch.dict(os.environ, {"PROXY_API_KEY": "custom_secret_123"}):
            import importlib
            import kiro.config as config_module
            importlib.reload(config_module)
            assert config_module.PROXY_API_KEY == "custom_secret_123"
