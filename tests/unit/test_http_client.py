...
class TestKiroHttpClientRequestWithRetry:
    ...
    @pytest.mark.asyncio
    async def test_429_returns_last_response_after_retries_exhausted(self, mock_auth_manager_for_http):
        """
        What it does: Verifies exhausted 429 retries return the final upstream response.
        Purpose: Ensure rate limits are not misreported as unknown 504 errors.
        """
        print("Setup: Creating KiroHttpClient...")
        http_client = KiroHttpClient(mock_auth_manager_for_http)

        first_response_429 = AsyncMock()
        first_response_429.status_code = 429
        first_response_429.aclose = AsyncMock()

        second_response_429 = AsyncMock()
        second_response_429.status_code = 429
        second_response_429.aclose = AsyncMock()

        final_response_429 = AsyncMock()
        final_response_429.status_code = 429
        final_response_429.aclose = AsyncMock()
        ...
        print("Action: Executing request with persistent 429 responses...")
        with patch.object(http_client, '_get_client', return_value=mock_client):
            with patch('kiro.http_client.get_kiro_headers', return_value={}):
        ...
        print("Verification: Final 429 response returned instead of raising 504...")
        assert response is final_response_429
        assert response.status_code == 429
        first_response_429.aclose.assert_called_once()
        second_response_429.aclose.assert_called_once()
        final_response_429.aclose.assert_not_called()

    @pytest.mark.asyncio
    async def test_5xx_returns_last_response_after_retries_exhausted(self, mock_auth_manager_for_http):
        """
        What it does: Verifies exhausted 5xx retries return the final upstream response.
        Purpose: Ensure upstream server errors preserve their original status code.
        """
        print("Setup: Creating KiroHttpClient...")
        http_client = KiroHttpClient(mock_auth_manager_for_http)

        first_response_503 = AsyncMock()
        first_response_503.status_code = 503
        first_response_503.aclose = AsyncMock()

        second_response_503 = AsyncMock()
        second_response_503.status_code = 503
        second_response_503.aclose = AsyncMock()

        final_response_503 = AsyncMock()
        final_response_503.status_code = 503
        final_response_503.aclose = AsyncMock()
        ...
        print("Action: Executing request with persistent 503 responses...")
        with patch.object(http_client, '_get_client', return_value=mock_client):
            with patch('kiro.http_client.get_kiro_headers', return_value={}):
                with patch('kiro.http_client.asyncio.sleep', new_callable=AsyncMock):
                    response = await http_client.request_with_retry(
        ...
        print("Verification: Final 503 response returned instead of raising 504...")
        assert response is final_response_503
        assert response.status_code == 503
        first_response_503.aclose.assert_called_once()
        second_response_503.aclose.assert_called_once()
        final_response_503.aclose.assert_not_called()
    ...
    @pytest.mark.asyncio
    async def test_streaming_429_returns_last_response_after_retries_exhausted(self, mock_auth_manager_for_http):
        """
        What it does: Verifies exhausted streaming 429 retries return the final upstream response.
        Purpose: Ensure Anthropic/OpenAI streaming paths receive the real rate-limit status.
        """
        print("Setup: Creating KiroHttpClient...")
        http_client = KiroHttpClient(mock_auth_manager_for_http)
        ...
        print("Action: Executing streaming request with persistent 429 responses...")
        with patch.object(http_client, '_get_client', return_value=mock_client):
            with patch('kiro.http_client.get_kiro_headers', return_value={}):
                with patch('kiro.http_client.asyncio.sleep', new_callable=AsyncMock):
                    response = await http_client.request_with_retry(
        ...
        print("Verification: Final streaming 429 response returned and prior responses closed...")
        assert response is final_response_429
        assert response.status_code == 429
        first_response_429.aclose.assert_called_once()
        second_response_429.aclose.assert_called_once()
