...
class TestModelsEndpoint:
    ...
    def test_models_include_glm_alias_when_configured(self, test_client, valid_proxy_api_key, monkeypatch):
        """
        What it does: Verifies glm-5 appears in the model list when GLM provider is enabled.
        Purpose: Ensure provider switch updates exposed model metadata.
        """
        print("Setup: Enabling GLM upstream provider...")
        monkeypatch.setattr("kiro.routes_openai.UPSTREAM_PROVIDER", "glm")
        monkeypatch.setattr(test_client.app.state.model_resolver, "get_available_models", lambda: ["glm-5"])
        ...
        print(f"Result: {response.json()}")
        assert response.status_code == 200
        assert response.json()["data"][0]["id"] == "glm-5"
        assert response.json()["data"][0]["owned_by"] == "zhipu"
        assert "GLM-5" in response.json()["data"][0]["description"]
    ...
    def test_models_owned_by_anthropic(self, test_client, valid_proxy_api_key):
        ...
        Purpose: Ensure model attribution matches the active upstream provider.
        """
        print("Action: GET /v1/models with valid auth...")
        response = test_client.get(
            "/v1/models",
        ...
        expected_owner = "anthropic"
        for model in response.json()["data"]:
            assert model["owned_by"] == expected_owner
...
class TestHTTPClientSelection:
    ...
    @patch('kiro.routes_openai.build_route_http_client')
...
class TestModelResolverIntegration:
    ...
    def test_chat_completions_passes_resolved_model_id_to_converter(
        self,
        mock_build_kiro_payload,
        ...
        """
        print("\n--- Test: Route passes resolved model ID to converter ---")

        monkeypatch.setattr(
            test_client.app.state.model_resolver,
            "resolve",
            lambda model: ModelResolution(
                internal_id="RESOLVED_INTERNAL_MODEL",
                source="hidden",
        ...
        mock_build_kiro_payload.return_value = {"conversationState": {}}
        ...
        assert mock_build_kiro_payload.called
        assert mock_build_kiro_payload.call_args.kwargs["resolved_model_id"] == "RESOLVED_INTERNAL_MODEL"
